...
def import_badges(conn):
    tree = ET.parse('xml/Badges.xml') #'input/badges.xml'
    rows = tree.getroot()
    for row in rows:
        attrs = row.attrib
...
def import_users(conn):
    tree = ET.parse('xml/Users.xml') # input/users.xml
    rows = tree.getroot()
    for row in rows:
        attrs = row.attrib
        cur = conn.execute('INSERT INTO users(id, reputation, creation_date, display_name, last_access_date, location, about_me, views, upvotes, downvotes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
def import_posts(conn):
    tree = ET.parse('xml/Posts.xml') #'input/posts.xml'
    rows = tree.getroot()
    for row in rows:
        attrs = row.attrib
        cur = conn.execute('INSERT INTO posts(id, post_type, accepted_answer_id, parent_id, creation_date, community_owned_date, closed_date, score, views, body, owner_user_id, last_editor_user_id, last_edit_date, last_activity_date, title, tags, answers, comments, favorites) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                     [
                         ...
                         int(attrs['ViewCount']) if 'ViewCount' in attrs else 0,
                         attrs['Body'],
...
def import_votes(conn):
    tree = ET.parse('xml/Votes.xml') #input/votes.xml
    rows = tree.getroot()
    for row in rows:
        attrs = row.attrib
...
def import_comments(conn):
    tree = ET.parse('xml/Comments.xml') # input/comments.xml
    rows = tree.getroot()
    for row in rows:
        attrs = row.attrib
