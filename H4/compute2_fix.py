...
with open('/data1/StackOverflow/_Robustness/H4/year_month.pickle', 'rb') as fr:
    year_month = pickle.load(fr)

# (Bottleneck)
# Load files in the folder.
for i in range(len(year_month)):
	for root, dirs, files in os.walk(f'/data1/StackOverflow/_Robustness/H4/lsm_data_fix/newbie/{year_month[i]}'): # parameter (change 'Final')
		folks = {}
		for folk in files:
	...
	# save via pickle
	with open(f'/data1/StackOverflow/_Robustness/H4/lsm_result_fix/newbie/{year_month[i]}.pickle', 'wb') as fw: # parameter (change 'Final_pickle')
		pickle.dump(df, fw)